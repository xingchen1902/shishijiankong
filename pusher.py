...
def push_ai_daily_report_to_telegram(date_str, report_text):
    """把 AI 日报推送到现有 Telegram 日常汇总目标。"""
    if not TELEGRAM_BOT_TOKEN:
        print("  [AI Telegram] 跳过: 未配置 BOT_TOKEN")
        return False
    message = f"📈 ARK AI每日分析\n{date_str}\n\n{report_text}"
    if len(message) > 4000:
    ...
    for chat_id in get_telegram_chat_ids():
        try:
            ...
            if not result.get("ok"):
                ok = False
                print(f"  [AI Telegram] 推送失败 chat_id={chat_id}: {result}")
        except Exception as exc:
            ok = False
            print(f"  [AI Telegram] 请求失败 chat_id={chat_id}: {exc}")
    if ok:
        print(f"  [AI Telegram] 推送成功 {date_str}")
    return ok


def push_release_top30_excel(date_str, release_type):
    """生成并把当日对应类型释放地址前30名 Excel 发到正能量小组。"""
    is_dynamic = release_type == "dynamic"
    label = "动态" if is_dynamic else "静态"
    event_type = "release_dynamic" if is_dynamic else "release_static"
    if not TELEGRAM_BOT_TOKEN:
        print(f"  [{label}释放排名] 跳过: 未配置 BOT_TOKEN")
        return False
    from static_release_report import build_xlsx, get_top_release_rows

    rows = get_top_release_rows(date_str, event_type)
    if not rows:
        print(f"  [{label}释放排名] {date_str} 无{label}释放数据，跳过文件发送")
        return True
    filename = f"{label}释放前30名_{date_str}.xlsx"
    caption = (
        f"📊 {date_str} {label}释放地址前30名\n"
    ...
    try:
        response = requests.post(
            ...
            files={"document": (filename, build_xlsx(date_str, rows, release_type), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")},
            timeout=60,
        )
        result = response.json()
        if result.get("ok"):
            print(f"  [{label}释放排名] 推送成功 {date_str} rows={len(rows)}")
            return True
        print(f"  [{label}释放排名] 推送失败: {result}")
    except Exception as exc:
        print(f"  [{label}释放排名] 请求失败: {exc}")
    return False


def push_static_release_top30_excel(date_str):
    """生成并发送当日静态释放地址前30名 Excel。"""
    return push_release_top30_excel(date_str, "static")


def push_dynamic_release_top30_excel(date_str):
    """生成并发送当日动态释放地址前30名 Excel。"""
    return push_release_top30_excel(date_str, "dynamic")
