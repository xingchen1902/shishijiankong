...
def get_today_data():
    ...
    row = conn.execute("""
        SELECT COALESCE(SUM(CASE WHEN lower(from_addr)='0x8501168656fcac4628f6910ccabea8b64ebe5bd4' THEN value ELSE 0 END),0),
               ...
               COALESCE(SUM(CASE WHEN lower(from_addr)='0x8366a748e02f730911cb5ab4fd049d2e1e0414b7' AND lower(to_addr)!='0x7dfad978e43d47bae564c2cbba88f280474a7c24' THEN value ELSE 0 END),0),
